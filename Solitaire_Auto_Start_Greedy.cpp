    ...
    while(true) {
        ...
        if(isPossibleToReachTargetAutoPlacedCards) {
            ...
            while(true) {
                ...
                // Flag used to indicae if a card was auto placed
                // If this is still false by the end of the outer for loop, the algorighm stops
                //bool didAutoPlaceCard = false;
                ...
                // If at least one card can be moved, move the top card of the card stack which has the highest "movement potential"
                ...
                // Get the card stack index to move and the ace pile index to move its top card to
                int bestCardStackIndex = cardMovementArray[0][2];
                int acePileToMoveToIndex = cardMovementArray[0][1];
                ...
                // Move the top card from the card stack with the highest number of potential card movements
                cardStacks[bestCardStackIndex][0]--;     // Pop the top card of the current card stack
                acePiles[acePileToMoveToIndex]++;        // Place it on the current ace pile
                
                // Increment the auto placed card counter
                numAutoPlacedCards++;
                
                // Print out the wanted card of each ace pile after the auto placement was made
